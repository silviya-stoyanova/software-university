class FilmStudio {
    ...
    casting(actor, role) {
        ...
        if (this.films.length) {
            ...
            for (let f of this.films) {
                ...
                // търси за кой филм е ролята
                let roles = f.filmRoles.filter((r) => r.role === role);

                // ако ролята съществува
                if (roles.length) {
    ...
    lookForProducer(film) {
        // проверява дали подадения филм съществува
        // и взима само първия срещнат (1вата част ако е филмова поредица)
...
let filmStudio = new FilmStudio('Silvi movies')
